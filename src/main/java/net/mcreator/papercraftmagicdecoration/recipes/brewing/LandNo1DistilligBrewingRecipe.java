...
public class LandNo1DistilligBrewingRecipe implements IBrewingRecipe {
	@SubscribeEvent
	public static void init(FMLCommonSetupEvent event) {
		event.enqueueWork(() -> BrewingRecipeRegistry.addRecipe(new LandNo1DistilligBrewingRecipe()));
	}
	...
	public boolean isIngredient(ItemStack ingredient) {
		return Ingredient.of(new ItemStack(Items.APPLE)).test(ingredient);
	}

	@Override
	public ItemStack getOutput(ItemStack input, ItemStack ingredient) {
		if (isInput(input) && isIngredient(ingredient)) {
			return new ItemStack(PapercraftMagicDecorationModBlocks.LAND_NO_1.get());
		}
