...
public class CookingInterruption2Procedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		if (!((world.getBlockState(BlockPos.containing(x, y - 1, z))).getBlock() == Blocks.MAGMA_BLOCK)) {
			...
			if (((Entity) world.getEntitiesOfClass(Player.class, AABB.ofSize(new Vec3(x, y, z), 8, 8, 8), e -> true).stream().sorted(new Object() {
				...
				_player.displayClientMessage(Component.literal((Component.translatable("Cooking on the Magma Block").getString())), true);
		}
	}
}
